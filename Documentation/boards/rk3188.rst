...
.. code-block:: sh
  ...
  make ARCH=arm rockchip_v7a_defconfig
  make ARCH=arm

Creating bootable SD card
^^^^^^^^^^^^^^^^^^^^^^^^^

This will require a DRAM setup blob and additional utilities, see below.
...
* Run "rk-makebootable FlashData images/barebox-radxa-rock.img rrboot.bin"
