...
u32 lpddr4_writectlconfig(const lpddr4_privatedata *pd, struct reginitdata *regs)
{
	u32 result;
	u32 i;
	...
	if (result == (u32)0) {
		for (i = 0; i < regs->num; i++) {
			u16 offset = regs->offset ? regs->offset[i] : i;
			result = (u32)lpddr4_writereg(pd, LPDDR4_CTL_REGS,
						      offset, regs->regs[i]);
		}
	}
	return result;
...
u32 lpddr4_writephyindepconfig(const lpddr4_privatedata *pd, struct reginitdata *regs)
{
	u32 result;
	u32 i;
	...
	if (result == (u32)0) {
		for (i = 0; i < regs->num; i++) {
			u16 offset = regs->offset ? regs->offset[i] : i;
			result = (u32)lpddr4_writereg(pd, LPDDR4_PHY_INDEP_REGS,
						      offset, regs->regs[i]);
		}
	}
	return result;
...
u32 lpddr4_writephyconfig(const lpddr4_privatedata *pd, struct reginitdata *regs)
{
	u32 result;
	u32 i;
	...
	if (result == (u32)0) {
		for (i = 0; i < regs->num; i++) {
			u16 offset = regs->offset ? regs->offset[i] : i;
			result = (u32)lpddr4_writereg(pd, LPDDR4_PHY_REGS,
						      offset, regs->regs[i]);
		}
	}
	return result;
