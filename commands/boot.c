...
BAREBOX_CMD_HELP_TEXT("- a filename under /env/boot/ followed by @ARGUMENT to pass ARGUMENT")
BAREBOX_CMD_HELP_TEXT("   as first argument to the script, e.g. mmc@1 runs /env/boot/mmc 1")
BAREBOX_CMD_HELP_TEXT("   ARGUMENT must start with a slash, dash, underscore, dot or")
BAREBOX_CMD_HELP_TEXT("   alphanumeric character")
